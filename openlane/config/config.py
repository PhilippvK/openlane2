...
from .flow import pdk_variables, scl_variables, flow_common_variables, flow_common_variables2
from .pdk_compat import migrate_old_config
from .preprocessor import preprocess_dict, preprocess_dict2, Keys as SpecialKeys
...
class GenIEConfig(GenericImmutableDict[str, Any]):
    ...
    def copy_filtered(
        ...
        variables: Set[str] = set([variable.name for variable in config_vars])
        if include_flow_variables:
            variables = variables.union(set([variable.name for variable in flow_common_variables2]))
    ...
    ## For Jupyter
    def _repr_markdown_(self) -> str:  # pragma: no cover
        title = "Interactive Configuration" if self == Config.current_interactive else "Configuration"
        values_title = "Initial Values" if self == Config.current_interactive else "Values"
        return (
            dedent(
                f"""
                ### {title}
